...
const CONVO_ID = "openai-demo";
const EVENT_NAME = "openai_chat";
...
<html>
  <head>
    <meta charset="utf-8" />
    <title>Raindrop OpenAI Chat</title>
  ...
  <body>
    <div class="container">
      <div class="scrollable" id="scrollable">
        <h1>OpenAI SDK Chat</h1>
        <p>Direct OpenAI SDK + raindrop-ai manual <code>withTool</code>/<code>withSpan</code> instrumentation.</p>
        ...
        <p style="font-size: 11px; color: #888; margin: .5rem 0 0;">Note: OpenAI reasoning models (gpt-5.x, o-series) ignore <code>temperature</code>, <code>topP</code>, and <code>seed</code>. <code>reasoning_effort</code> is dropped when <code>tools</code> are on (chat.completions limitation \u2014 use /v1/responses).</p>
...
export function createApp(): Express {
  ...
  app.post("/api/chat", async (req, res) => {
    ...
    if (!client) {
      res.status(500).type("text/plain").send("OPENAI_API_KEY is required");
      return;
    }
    ...
    try {
      const finalText = await interaction.withSpan({ name: "openai.chat" }, async () => {
        const messages: any[] = [
          { role: "system", content: systemPrompt },
          ...(incoming as any[]),
        ];
        const baseParams: Record<string, unknown> = { model: requestedModel };
        for (const [src, dst] of [
          ...
          const v = body[src];
          if (typeof v === "number" && Number.isFinite(v)) baseParams[dst] = v;
        }
        if (reasoningEffort && !useTools) baseParams.reasoning_effort = reasoningEffort;
          ...
          for (const tc of toolCalls) {
            ...
            const result = handler
              ? await interaction.withTool(
                  { name: tc.name, inputParameters: parsedArgs },
                  () => handler(parsedArgs, interaction),
                )
      ...
      interaction.finish({ output: finalText, model: requestedModel });
